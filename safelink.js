 ...
 *
 *  v2.0 — إصلاح: ظهور الأزرار على كل أنواع الصفحات
 *         + إدراج ذكي في أعلى المحتوى
 *         + سكرول تلقائي نحو الحاوية
 *         + دعم أفضل لمحددات بلوجر
 *
 * ============================================================
 */
...
(function () {
    ...
    /* دمج الإعدادات */
    var C = {};
    var uc = window.SAFELINK_CONFIG || {};
    for (var k in DEFAULTS) {
        C[k] = (uc[k] !== undefined) ? uc[k] : DEFAULTS[k];
    }
    ...
    /* ════════════════════════════════════════════════════════════
     *  إيجاد مكان الإدراج — محسّن لكل أنواع صفحات بلوجر
     * ════════════════════════════════════════════════════════════
     *
     *  الأولوية:
     *  1. داخل مقال مفرد (صفحة post)
     *  2. داخل منطقة المقالات (صفحة رئيسية)
     *  3. داخل main
     *  4. بداية body
     *
     * ════════════════════════════════════════════════════════════ */
    ...
    function findInsertPoint() {
        /* 1 — مقال مفرد: .post-body أو .entry-content أو .post-content */
        var postSels = [
            '.post-body',
            '.entry-content',
            '.post-content',
        ...
        for (var i = 0; i < postSels.length; i++) {
            var el = document.querySelector(postSels[i]);
            if (el) {
                log('Found post body:', postSels[i]);
                return { target: el, type: 'post' };
            }
        }

        /* 2 — منطقة المقالات (صفحة رئيسية) */
        var blogSels = [
            '.blog-posts',
            '#Blog1 .blog-posts',
            '#Blog1',
        ...
        for (var i = 0; i < blogSels.length; i++) {
            var el = document.querySelector(blogSels[i]);
            if (el && el.firstChild) {
                log('Found blog area:', blogSels[i]);
                return { target: el, type: 'blog' };
            }
        }

        /* 3 — main */
        var main = document.querySelector('main') || document.querySelector('#main') || document.querySelector('.main-content');
        if (main) {
            log('Found main element');
            return { target: main, type: 'main' };
        }
    ...
    function insertWrapper(wrapper, position) {
        var pt = findInsertPoint();
        var target = pt.target;
        var type = pt.type;

        if (type === 'post') {
            /* صفحة مقال — إدراج قبل/بعد محتوى المقال */
            if (position === 'header') {
            ...
            /* صفحة رئيسية أو أخرى — إدراج في أعلى المنطقة */
    ...
    function injectCSS() {
        ...
        s.id = 'sl-css';
        s.textContent = '\
.sl-wrap{background:linear-gradient(135deg,#f8f9fa,#e9ecef);border:2px solid #0d6efd;border-radius:12px;padding:24px 20px;margin:20px 0;text-align:center;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Noto Sans SC",sans-serif;direction:rtl;box-shadow:0 4px 16px rgba(13,110,253,.15);position:relative;z-index:100}\
.sl-steps{display:flex;justify-content:center;gap:8px;margin-bottom:16px}\
.sl-step{width:30px;height:30px;border-radius:50%;background:#e9ecef;color:#6c757d;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700;transition:all .3s}\
...
.sl-timer{font-size:16px;color:#495057;margin-bottom:10px;min-height:24px;font-weight:600}\
.sl-bar{width:100%;max-width:320px;height:8px;background:#e9ecef;border-radius:4px;overflow:hidden;margin:0 auto 16px}\
.sl-prog{height:100%;background:linear-gradient(90deg,#0d6efd,#6610f2);border-radius:4px;width:0%;transition:width 1s linear}\
.sl-hint{font-size:14px;color:#6c757d;margin-top:12px;animation:sl-b 2s infinite}\
...
';
    ...
    function buildUI() {
        ...
        places.forEach(function (p) {
            ...
            /* الزر — دائماً مرئي لكن معطل حتى يكتمل التايمر/السكرول */
